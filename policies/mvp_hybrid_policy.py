...
class HybridMlpPolicy(ActorCriticPolicy):
    ...
    def act(self, obs, rnn_hxs=None, rnn_masks=None, deterministic=False):
        ...
        act_param_logprob = [act_param_dist[i].log_prob(act_params[i].squeeze(dim=-1)).unsqueeze(dim=-1) for i in range(self.act_dim)]
    ...
    def evaluate_actions(self, obs, rnn_hxs, rnn_masks, actions):
        ...
        act_param_logprob = [act_param_dist[i].log_prob(act_params[:, i]) for i in range(self.act_dim)]
